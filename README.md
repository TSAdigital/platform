...
В случае успешного завершения процесса импорта в папке `web/export` будет автоматически создан файл `users_*.txt`, содержащий учетные данные пользователей.

После сохранения файла `users_*.txt` в безопасном месте настоятельно рекомендуется удалить его из папки `web/export` в целях обеспечения безопасности. Также, по завершении работы, вы можете удалить файл `import.xlsx` из папки `web/import`.
...
### 5. Импорт данных из РЭМД

### Перед импортом убедитесь, что на сервере настроены параметры PHP и сервера для обработки большого объема данных
#### Рекомендуемые параметры, которые стоит проверить
- `max_execution_time`
- `memory_limit`
- `upload_max_filesize` и `post_max_size`

### 1. Переход и подготовка данных

1. Перейдите в систему РЭМД: [http://remd.egisz.rosminzdrav.ru/](http://remd.egisz.rosminzdrav.ru/)

2. Настройка поиска:

   - В разделе **"Записи реестра"**
   - В поле **"РМИС/МИС"** выберите: **ГИСЗ субъекта.**
   - Укажите диапазон даты и времени регистрации документа (максимум — около одной недели).
   - Нажмите **"Найти"**.

3. Настройка отображения результатов:

   - После поиска нажмите значок шестеренки.
   - В меню **"Скрытие столбцов"** установите галочки напротив столбцов (Вид документа, Дата регистрации, Подписавшие сотрудники).
   - Нажмите **"Выгрузить"**.
   - Выберите формат: **XLSX**, и кодировку: **UTF-8**.

---

### 2. Подготовка и импорт файла

- Переименуйте полученный файл в `remd.xlsx`.
- Перенесите файл на сервер в папку `@web/import`.
